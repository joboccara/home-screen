import random

def get_fact():
    return LIBRARY[random.randint(0, len(LIBRARY) - 1)]

LIBRARY = [
    "Chuck Norris was once bitten by a king cobra; after five days of agonizing pain, the cobra died.",
    "When Chuck Norris does push-ups, he doesn't push himself up, he pushes the Earth down.",
    "Chuck Norris has a polar bear rug at home. It's not dead; it's just afraid to move.",
    "Chuck Norris actually died 20 years ago, but death hasn't built up the courage to tell him yet.",
    "Chuck Norris doesn't do push-ups. He pushes the Earth down.",
    "When Chuck Norris enters a room, he doesn't turn the lights on. He turns the dark off.",
    "The Boogeyman checks his closet for Chuck Norris before going to bed.",
    "Chuck Norris can slam a revolving door.",
    "Chuck Norris makes onions cry.",
    "Chuck Norris can kill two stones with one bird.",
    "Death had a near-Chuck-Norris experience.",
    "Ghosts sit around the campfire and tell Chuck Norris stories.",
    "Un jour, Chuck Norris a jeté une grenade sur une bande de malfrats : il y a eu six morts ; puis la grenade a explosé.",
    "Il n'y a plus de vie sur Mars depuis que Chuck Norris y est allé.",
    "Chuck Norris a déjà compté jusqu'à l'infini. Deux fois.",
    "Chuck Norris peut diviser par zéro.",
    "Chuck Norris a fini son forfait illimité.",
    "Chuck Norris a lu le texte de la Constitution européenne.",
    "Chuck Norris comprend les textes du groupe Magic System.",
    "Chuck Norris a compris l'histoire du film Inception.",
    "Certaines personnes portent un pyjama Superman. Superman porte un pyjama Chuck Norris.",
    "Jésus est né en 1940 avant Chuck Norris.",
    "La gravité, c'est la force qui fait tenir la Terre sous Chuck Norris.",
    "Beaucoup de gens craignent de prendre l'avion mais tous les avions craignent de prendre Chuck Norris.",
    "L'avenir se demande parfois ce que Chuck Norris lui réserve.",
    "Chuck Norris peut faire un moine avec un habit",
    "Chuck Norris ne ment pas, c'est la vérité qui se trompe.",
    "Si Chuck Norris dort la lumière allumée, c'est parce que le noir a peur de lui.",
    "Un jour, Chuck Norris a frôlé la mort ; elle ne s'en est jamais remise.",
    "Chuck Norris ne se mouille pas, c'est l'eau qui se Chuck-Norrise.",
    "Le Diable a vendu son âme à Chuck Norris.",
    "Chuck Norris a retrouvé Ornicar.",
    "L'évidence se rend à Chuck Norris.",
    "Les dinosaures ont regardé Chuck Norris de travers une fois. Une seule fois.",
    "Les requins regardent Les Dents de Chuck Norris à leur soirée de film d'horreur."
]